...
#define ATOL RCONST(1.0e-4)   // 1.0e-4
...
#define P1_NEQ        2
#define P1_ETA        RCONST(3.0)   // 3.0
#define P1_NOUT       80
#define P1_T0         RCONST(0.0)
#define P1_T1         RCONST(1.39283880203)  // 1.39283880203
#define P1_DTOUT      RCONST(0.2214773875)  // 2.214773875
#define P1_TOL_FACTOR RCONST(1.0e0)   // 1.0e4
...
#define P2_TOL_FACTOR RCONST(1.0e3) // 1.0e3
...
/*
  char filename[14];
  strncpy(filename, "cvDireDemo.out", 8);
  SUNEXPORT_DATA fileclass=SUNEXPORT_DATA(DIAG,filename);  
  dpair values;
  li_doubles vectorvalues;
*/
  y = NULL;
  A = NULL;
  LS = NULL;
  ...
  printf("CVodeCreate finished\n");
  for (miter=FUNC; miter <= DIAG; miter++) {
    printf("module FUNC\n");
    ero = ZERO;
    NV_Ith_S(y,0) = TWO;
    printf("macro NV_Ith for zero works\n");
    NV_Ith_S(y,1) = ZERO;

    firstrun = (miter==FUNC);
    if (firstrun) {
      printf("CVodeInit begin\n");
      flag = CVodeInit(cvode_mem, f1, P1_T0, y);
      if(check_flag(&flag, "CVodeInit", 1)) return(1);
      printf("CVodeInit finished\n");      
...
*/
 ...
 // frederic:
 // int fcv=fileclass.exportToDisk(vectorvalues);
